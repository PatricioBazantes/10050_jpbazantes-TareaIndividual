...
class matricula{
	...
	if($bandera){
		...
		$html = '
		<form name="Form_matricula" method="POST" action="matricula.php" enctype="multipart/form-data">
		...
		<input type="hidden" name="op" value="' . $op  . '">
			...
			<table border="2" align="center" class="table-sm">
				...
				<tr>
					<th colspan="2" style="background-color:black;"><a href="matricula.php" class="Boton" id="regresar">Regresar</a></th>
				</tr>
			</table>';
		return $html;
		}
	...
	public function get_list(){
		...
		<table border="1" align="center" class="table-sm table-dark">
			...
			<tr>
				<th colspan="8"><a href="matricula.php?d=' . $d_new_final . '" class="Boton" id="nuevo">Nuevo</a></th>
			</tr>
			<tr>
				<th>Placa</th>
				<th>Marca</th>
			...
			*/
				...
				$html .= '
					<tr>
						...
						<td>' . $row['anio'] . '</td>
						<td>' . $row['avaluo'] . '</td>
						<td><a href="" class="Boton" id="matriculacion">Matriculacion</a></td>
					</tr>';
		    }
		}else{
		...
		<div>    
			<a href="../../index.html">
				<img id="inicioImg" src="../../Recursos/img/Inicio.png" alt="Inicio">
			</a>
  		</div>';
		return $html;
	}
...
public function save_vehiculo(){
	...
	VALUES (NULL, '$this->placa', '$this->marca', '$this->motor', '$this->chasis', '$this->combustible', '$this->anio', '$this->color', '$this->foto','$this->avaluo');";
...
public function get_detail_vehiculo($id){
	...
	public function update_vehiculo(){
		...
		$id = $_POST['id'];
		//como se va al vehiculo, se pierde el dato, asi que se vuelve a instanciar y carga el dato de nuevo
            $this->placa = $_POST['placa']; //Los datos que estan capturados van a cambiar por el nuevo envio
            $this->marca = $_POST['marca']; //de datos, enviados por post
            $this->motor = $_POST['motor'];
            $this->chasis = $_POST['chasis'];
            $this->combustible = $_POST['combustible'];
            $this->anio = $_POST['anio'];
...
//***************************************************************************************************************************
	...
	private function _message_error($tipo){
		...
		<table border="0" align="center" class="table-sm table-dark">
			...
			<tr>
				<th><a href="matricula.php" class="Boton" id="regresar">Regresar</a></th>
			</tr>
		</table>';
		return $html;
	}
